...
export default function ArtistAdminProvider(props: ChildrenProps) {
  ...
  const deleteArtist = useCallback(
    async (artist: any, messageApi: any) => {
  ...
  const updateArtist = useCallback(
    async (data: any, artist: any, messageApi: any) => {
  ...
  const memoProvider = useMemo(
    ...
    [artistsState, postArtist, deleteArtist, updateArtist]
  );

  return <ArtistAdminContext.Provider value={memoProvider}>{props.children}</ArtistAdminContext.Provider>;
}
