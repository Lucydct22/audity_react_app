...
export default function PlaylistAdminProvider(props: ChildrenProps) {
	...
	const postPlaylist = useCallback(async (playlist: any, messageApi: any) => {
		message.open({ type: 'loading', content: `Creating playlist '${playlist.name}'` })
		const token = await getAccessTokenSilently()
		action.postPlaylistAction(dispatch, playlist, token, messageApi)
	}, []);

	const deletePlaylist = useCallback(async (playlist: any, messageApi: any) => {
		message.open({ type: 'loading', content: `Removing playlist` })
		const token = await getAccessTokenSilently()
		if (isAuthenticated && token && playlist) {
			action.deletePlaylistAction(dispatch, playlist, token, playlistsState, messageApi)
		}
	}, [playlistsState]);
