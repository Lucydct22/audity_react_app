...
export default function TrackAdminProvider(props: ChildrenProps) {
  ...
  const deleteTrack = useCallback(
    async (track: any, messageApi: any) => {
  ...
  const updateTrack = useCallback(
    async (data: any, track: any, messageApi: any) => {
  ...
  const memoProvider = useMemo(
    ...
    [tracksState, postTrack, deleteTrack, updateTrack]
  );

  return <TrackAdminContext.Provider value={memoProvider}>{props.children}</TrackAdminContext.Provider>;
}
