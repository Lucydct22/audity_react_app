...
export default function UserAdminProvider(props: ChildrenProps) {
  ...
  const deleteUser = useCallback(
    async (userId: any, messageApi: any) => {
  ...
  const updateUserRole = useCallback(
    async (role: any, userId: any, messageApi: any) => {
  ...
  const memoProvider = useMemo(
    ...
    [userState, deleteUser, updateUserRole]
  );

  return <UserAdminContext.Provider value={memoProvider}>{props.children}</UserAdminContext.Provider>;
}
