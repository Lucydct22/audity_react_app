...
export default function AlbumAdminProvider(props: ChildrenProps) {
  ...
  const deleteAlbum = useCallback(
    async (album: any, messageApi: any) => {
  ...
  const updateAlbum = useCallback(
    async (data: any, album: any, messageApi: any) => {
  ...
  const memoProvider = useMemo(
    ...
    [albumsState, postAlbum, deleteAlbum, updateAlbum]
  );

  return <AlbumAdminContext.Provider value={memoProvider}>{props.children}</AlbumAdminContext.Provider>;
}
