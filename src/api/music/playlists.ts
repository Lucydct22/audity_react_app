...
export const putTrackToPlaylistApi = async (
  token: any,
  playlistId: string,
  trackId: string
): Promise<any> => {
  ...
  const response = await fetch(
    `${basePath}/playlist-add-track/playlists/${playlistId}/tracks/${trackId}`,
    params
  );
  const data = await response.json();
  return data;
...
export const deleteTrackFromPlaylist = async (
  ...
  const response = await fetch(
    `${basePath}/delete-track-from-playlist/${playlistId}/tracks/${trackId}`,
    params(token)
  );
  const data = await response.json();
  return data;
...
export const updatePlaylistPublicAccessibleApi = async (
  playlistId: string,
  publicAccessible: any,
  token: any
): Promise<any> => {
  const params = {
    method: 'PUT',
