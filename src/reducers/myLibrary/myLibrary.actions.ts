...
import {
  getPlaylistByUserApi,
  getPlaylistsLikedByUserApi,
  likeDislikePlaylistApi,
  postPlaylistApi,
  putTrackToPlaylistApi,
} from 'api/music/playlists';
import {
  getPrivateTracksApi,
  getTracksLikedByUserApi,
  likeDislikeTrackApi,
  postPrivateTrackApi,
  updateTrackAudioApi,
} from 'api/music/tracks';
...
export async function postPlaylistAction(
  dispatch: any,
  token: any,
  name: any,
  description: string,
...
export async function putTrackToPlaylistAction(
  dispatch: any,
  token: any,
  playlistId: string,
  trackId: string
  ...
  try {
    ...
    if (response.status === 200) {
      if (response.haveLike === true) {
        ...
        const filterUserState = userState.artists.content.filter(
          (artistF: any) => artistF._id !== artist._id
        );
        return dispatch({
  ...
  try {
    ...
    if (response.status === 200) {
      if (response.haveLike === true) {
        ...
        const filterUserState = userState.playlists.content.filter(
          (playlistF: any) => playlistF._id !== playlist._id
        );
        return dispatch({
