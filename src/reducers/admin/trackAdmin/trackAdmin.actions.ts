...
export async function postTrackAction(dispatch: any, data: any, token: any, messageApi: any) {
  try {
    ...
    const postTrackAudio: any = await api.updateTrackAudioApi(
      postTrack.track._id,
      data.audio.file.originFileObj,
      token
    );
    const audio = new Audio();
    audio.src = postTrackAudio.track.audioUrl;
    const duration: any = await getDuration(audio);
    const trackToUpdate = await api.updateTrackApi(
      postTrackAudio.track._id,
...
export async function updateTrackAction(
  ...
  try {
    ...
    if (data.image?.file.originFileObj) {
      const trackImageToUpdate = await api.updateTrackImageApi(
        track._id,
    ...
    if (data.audio?.file.originFileObj) {
      const trackAudioToUpdate = await api.updateTrackAudioApi(
        track._id,
        data.audio.file.originFileObj,
        token
      );
    ...
    if (
      (data.name && data.name !== track.name) ||
      data.genres ||
      data.album ||
      data.artists ||
      data.playlists
