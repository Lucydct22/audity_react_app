...
export default function ArtistPage() {
  ...
  useEffect(() => {
    let isMounted = true;
    artistId &&
      getArtistByIdApi(artistId.toString()).then((res: any) => {
    ...
    if (artist) {
      artist.tracks.map((track: any) => {
        getTrackByIdApi(track._id.toString()).then((res: any) => {
          setTracksOfArtist((tracksOfArtist: any) => [...tracksOfArtist, res.track]);
        });
      });
    }
    return () => {
  ...
  return (
    <HelmetSEO title={`Artist | ${artist?.name}`} description="Audity Artist Page">
