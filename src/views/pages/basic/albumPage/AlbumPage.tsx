...
export default function AlbumPage() {
  ...
  useEffect(() => {
    let isMounted = true;
    albumId &&
      getAlbumByIdApi(albumId.toString()).then((res: any) => {
    ...
    if (album) {
      album.tracks.map((track: any) => {
        getTrackByIdApi(track._id.toString()).then((res: any) => {
          setTracksOfAlbum((tracksOfAlbum: any) => [...tracksOfAlbum, res.track]);
        });
      });
    }
    return () => {
  ...
  return (
    <HelmetSEO title={`Album | ${album?.name}`} description="Audity Album Page">
