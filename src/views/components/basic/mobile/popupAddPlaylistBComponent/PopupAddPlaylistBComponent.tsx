  ...
  return (
    <>
      <div className="render-playlist-mini-container">
        {playlists.userContent.map(
          (playlist: { _id: string; name: string; tracks: string[]; cover: string }) => {
            return (
              <div
                className="render-playlist-mini"
                key={playlist._id}
                onClick={() => handlePutTrackToPlaylist(playlist._id, playlist.name)}
              >
                ...
                <div className="render-playlist-mini__info">
                  ...
                  <p className="render-playlist-mini__info__tracks">
                    {t('musicpage_albumtracks')}: {playlist.tracks.length}
                  </p>
                </div>
              </div>
            );
...
const PopupAddPlaylistBComponent = ({ onClose }: any) => {
  ...
  const notify = () =>
    toast(t('player_component_popover_playlist'), {
      position: 'top-right',
      autoClose: 3000,
        ...
        <>
          <div className="popupAddPlaylistBComponent">
            <div className="popupAddPlaylistBComponent__container">
              <button onClick={onClose} className="player-mobile-add-to-playlist__btn">
              ...
              <div className="player-mobile-add-to-playlist">
                <div className="player-mobile-add-to-playlist__add">
                  <button onClick={handleModal}>
                    ...
                    <span className="player-mobile-add-to-playlist__add__text">
                      {t('player_component_popover_add_playlist')}
                    </span>
                  </button>
                </div>
