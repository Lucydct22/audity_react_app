...
export default function AlbumsBComponent({ albums }: any) {
  ...
  const filterAlbums = (albums: any) =>
    albums.filter((album: any) => new RegExp(`.*${albumsQuery}.*`, 'i').test(album.name));

  const renderAlbums = () =>
    albumsQuery
      ? filterAlbums(albums).map((album: any) => <RenderAlbum key={album._id} album={album} />)
  ...
  return (
    <div className="albums-page-content">
      ...
      <h1>Album Page</h1>
      <div className="albums-page-content__grid">{albums && renderAlbums()}</div>
    </div>
  );
}
