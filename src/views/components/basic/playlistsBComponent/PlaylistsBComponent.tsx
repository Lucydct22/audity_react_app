...
export default function PlaylistsBComponent({ playlists }: any) {
  ...
  const filterPlaylists = (playlist: any) =>
    playlist.filter((playlist: any) => new RegExp(`.*${playlistsQuery}.*`, 'i').test(playlist.name));

  const navigate = useNavigate();

  const renderPlaylists = () =>
    playlistsQuery
      ? filterPlaylists(playlists).map((playlist: any) => (
          <RenderPlaylist key={playlist._id} playlist={playlist} />
        ))
  ...
  return (
    <div className="playlist-page-content">
      ...
      <h1>Playlist Page</h1>
      <div className="playlist-page-content__grid">{playlists && renderPlaylists()}</div>
    </div>
  );
}
