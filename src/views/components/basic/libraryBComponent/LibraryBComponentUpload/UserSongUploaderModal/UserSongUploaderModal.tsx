...
const UserSongUploaderModal = ({ audio, isOpened, onClose, setUploadedAudio }: any) => {
  ...
  return (
    <dialog
      ...
      onCancel={onClose}
      onClick={onClose}
      className={isOpened ? 'modal-upload-song modal-song-open' : 'modal-upload-song modal-song-close'}
    >
      <div onClick={preventAutoClose} className="modal-upload-song--content">
        <h2>{t('library_upload_modal_title')}</h2>
        <button onClick={onClose} className="modal-upload-song--content__close">
          <VscChromeClose />
        </button>
        ...
        <button onClick={proceedAndClose} className="modal-upload-song--content__save">
          Save
        </button>
      </div>
    </dialog>
