    ...
    <>
      ...
      <div className="album-page">
        ...
        <section className="album-page__section">
          <h1>{album?.name}</h1>
          <p>
            {album?.tracks.length} {t('page_pnumber')}
          </p>
          <div className="album-page__section--buttons">
            <button className="album-page__section--buttons__play">
              {trackData.isPlaying ? (
            ...
            <button className="album-page__section--buttons__like" onClick={() => likeDislikeAlbum(album)}>
              {!songLike ? <AiFillHeart color="#ef5466" /> : <AiOutlineHeart />}
            </button>
          </div>
        </section>
      </div>
