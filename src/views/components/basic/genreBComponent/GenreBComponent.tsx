          ...
          <div className="genre-page__section">
            ...
            <div className="genre-page__section--content">
              {albums.map((album: any) => (
                <RenderAlbum key={album._id} album={album} />
              ))}
            </div>
          </div>
        )}
          ...
          <div className="genre-page__section">
            ...
            <div className="genre-page__section--content">
              {artists.map((artist: any) => (
                <RenderArtist key={artist._id} artist={artist} />
              ))}
            </div>
          </div>
        )}
        {playlists.length <= 0 && albums.length <= 0 && artists.length <= 0 && (
          <h3>
            No content found under genre {genre?.name}.<br /> <Link to={'/'}>Go back to home page</Link>
          </h3>
        )}
      </div>
    </>
