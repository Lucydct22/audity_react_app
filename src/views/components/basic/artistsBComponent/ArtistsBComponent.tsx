...
export default function ArtistsBComponent({ artists }: any) {
  ...
  const filterArtists = (artists: any) =>
    artists.filter((artist: any) => new RegExp(`.*${artistsQuery}.*`, 'i').test(artist.name));

  const renderArtists = () =>
    artistsQuery
      ? filterArtists(artists).map((artist: any) => <RenderArtist key={artist._id} artist={artist} />)
  ...
  return (
    <div className="artists-page-content">
      ...
      <h1>Artist Page</h1>
      <div className="artists-page-content__grid">{artists && renderArtists()}</div>
    </div>
  );
}
