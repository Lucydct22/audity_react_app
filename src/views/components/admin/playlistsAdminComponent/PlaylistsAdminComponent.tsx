          ...
          <List.Item
            actions={[
              <Select
                ...
                onChange={(publiAccessible) =>
                  updatePlaylistPublicAccessible(item, publiAccessible, messageApi)
                }
              />,
              <Popconfirm
                ...
                style={{ padding: '10px' }}
              >
                <Button className="delete-button" type="link" key="list-loadmore-remove" danger>
                  <DeleteOutlined />
                </Button>
              </Popconfirm>,
              <a key="list-loadmore-edit" onClick={() => navigate(`/admin/playlist-form/${item._id}`)}>
                Edit
              </a>,
            ]}
          >
            <Skeleton avatar title={false} loading={item.loading} active>
              <List.Item.Meta avatar={<Avatar src={item.imageUrl} />} title={<p>{item.name}</p>} />
            </Skeleton>
          </List.Item>
        )}
      />
      <FloatButton icon={<PlusOutlined />} type="primary" onClick={() => navigate('/admin/playlist-form')} />
    </>
  );
}
